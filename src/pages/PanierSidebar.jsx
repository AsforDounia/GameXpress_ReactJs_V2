...
const PanierSidebar = ({ isOpen, onClose }) => {
  const { getCart, cartDetails, updateQty, removeFromCart} = useCart();
  const { user } = useAuth();
    ...
    >
          ...
          (cartDetails.items.map((item) =>
            <div key={item.id} className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleUpdateQty(item.iproduct_id, item.quantity - 1)}
      ...
      <div className="p-4 border-t">
        <div className="flex justify-between font-semibold mb-3">
          <span>Total TTC :</span>
          <span>{(cartDetails.total_final).toFixe(2)} €</span>
        </div>
        <a
          //   href="/panier"
